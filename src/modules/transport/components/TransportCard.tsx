import { motion } from 'framer-motion';
import { ClockAlert, Fuel, Gauge, Grid2x2, Heart, Timer, Users, Warehouse } from 'lucide-react';
import { observer } from 'mobx-react-lite';
import { useState } from 'react';
import { Link } from 'react-router-dom';

import placeholder from '@/assets/sorryPlaceholder.svg';
import { ShowroomLabels } from '@/constants/transportLabels';
import Transport, { TransportClass } from '@/interfaces/Transport.interface';
import { userStore } from '@/store/user';

import styles from './TransportCard.module.scss';

interface TransportCardProps {
	transportData: Transport[];
	transportType: TransportClass;
}

export const TransportCard = observer(({ transportData, transportType }: TransportCardProps) => {
	const [likedTransport, setLikedTransport] = useState<Record<number, boolean>>({});
	const { isAuth } = userStore;

	const handleLike = (id: number) => {
		setLikedTransport((prev) => ({
			...prev,
			[id]: !prev[id],
		}));
	};

	return (
		<>
			{transportData.map((transport) => (
				<motion.div
					className={styles.transportCard}
					key={transport.id}
					initial={{ opacity: 0, y: 5 }}
					animate={{ opacity: 1, y: 0 }}
					transition={{ duration: 0.3 }}
					whileHover={{ scale: 1.01, transition: { duration: 0.07 } }}>
					<Link
						key={transport.id}
						to={'/transport/' + transport.id}
						//  onClick={handleCardClick}
					>
						<div className={styles.imageWrapper}>
							<img
								src={`/uploads/transport/cards/${transport.uniqueName}.webp`}
								alt={transport.name}
								className={styles.transportImage}
								loading="lazy"
								onError={(e) => {
									(e.currentTarget as HTMLImageElement).src = placeholder;
								}}
							/>
							{isAuth && (
								<button
									className={styles.likeButton}
									onClick={(e) => {
										e.stopPropagation();
										e.preventDefault();
										handleLike(transport.id);
									}}>
									{/* <Heart
										color={likedTransport[transport.id] ? '#a51d2d' : '#fff'}
										fill={likedTransport[transport.id] ? '#a51d2d' : 'none'}
									/> */}
									<Heart
										color={likedTransport[transport.id] ? '#c11b2eff' : '#000000ff'}
										fill={likedTransport[transport.id] ? '#c11b2eff' : 'white'}
									/>
								</button>
							)}
						</div>

						<div className={styles.headerCard}>
							<h2>{transport.name}</h2>
						</div>

						<div className={styles.details}>
							<div className={styles.leftColumn}>
								<div className={styles.detailItem}>
									<Gauge className={styles.icon} color="#ffffff" />
									<span>{transport.fullSpeed} км/ч</span>
								</div>

								<div className={styles.detailItem}>
									<Timer className={styles.icon} color="#ffffff" strokeWidth={1.5} />
									<span>0-100 {transport.speed100Time} с.</span>
								</div>

								<div className={styles.detailItem}>
									<ClockAlert className={styles.icon} color="#ffffff" strokeWidth={1.5} />
									<span>0-MAX {transport.speedMaxTime} с.</span>
								</div>
							</div>

							<div className={styles.rightColumn}>
								<div className={styles.detailItem}>
									<Fuel className={styles.icon} color="#ffffff" />
									<span> {transport.volumeTank} л.</span>
								</div>

								<div className={styles.detailItem}>
									<Warehouse className={styles.icon} color="#ffffff" strokeWidth={1.5} />
									<span>{ShowroomLabels[transport.showroom ?? 'MERCURY_AUTO']}</span>
								</div>

								{transportType === TransportClass.PUBLIC ? (
									<div className={styles.detailItem}>
										<Users className={styles.icon} color="#ffffff" strokeWidth={1.5} />
										<span> {transport.seats}</span>
									</div>
								) : (
									<div className={styles.detailItem}>
										<Grid2x2 className={styles.icon} color="#ffffff" strokeWidth={1.5} />
										<span> {transport.slots} сл.</span>
									</div>
								)}
							</div>
						</div>
					</Link>
					{transportType === TransportClass.CONTAINER ? (
						<div className={styles.price}>
							<span>Контейнер</span>
						</div>
					) : transportType === TransportClass.EXCLUSIVE ? (
						<div className={styles.price}>
							<span>Эксклюзив</span>
						</div>
					) : (
						<div className={styles.price}>
							{/* <button
								className={styles.like}
								onClick={() => handleLike(transport.id)}
							>
								{likedTransport[transport.id] ? (
								<Heart color='#a51d2d' fill='#a51d2d' />
								) : (
								<Heart color='#241f31' />
								)}
							</button> */}
							<span>
								{new Intl.NumberFormat('ru-RU', { useGrouping: true }).format(transport.price ?? 0)}{' '}
								₽
								<span className={styles.tooltip}>
									Актуальная гос. цена:{' '}
									{new Intl.NumberFormat('ru-RU', { useGrouping: true }).format(
										(transport.price ?? 0) * 0.9,
									)}{' '}
									₽
								</span>{' '}
							</span>
						</div>
					)}
				</motion.div>
			))}
		</>
	);
});
